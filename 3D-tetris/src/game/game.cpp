...
glm::vec3 Coord3D::getTranslateVec3()
{
	return -glm::vec3((float)x * 2, (float)y * 2, (float)z * 2);
}

std::vector<Coord3D> FallingShape::getCoords()
{
	...
	for (auto coord : *shape)
	{
		...
		if (r1 < 0) coord.x = -coord.x;
		if (r2 < 0) coord.y = -coord.y;
		if (r3 < 0) coord.z = -coord.z;
...
Game::Game() : moved(false), score(0), pushed(false)
{
...
void Game::moveFalling(int x, bool d)
{
	if (moved) return;
	bool flag = true;
	for (auto coord : falling->getCoords())
...
void Game::rotateFalling(int a, bool r)
{
	if (moved) return;
	int temp = 0;
